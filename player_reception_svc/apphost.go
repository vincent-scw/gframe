...
import (
	...
	e "github.com/vincent-scw/gframe/player_reception_svc/event"
)
...
func main() {
	...
	player.Use(jwtHandler.Serve)
	{
		player.Post("/in", func(ctx iris.Context) {
			...
			status := handleUserReception(p, authToken, events.EventIn)
			ctx.StatusCode(status)
		})
		player.Post("/out", func(ctx iris.Context) {
			authToken := jwtHandler.Get(ctx)
			status := handleUserReception(p, authToken, events.EventOut)
			ctx.StatusCode(status)
		})
	}
...
func handleUserReception(p *k.Producer, authToken *jwt.Token, t events.Status) int {
	user, err := e.NewEvent(authToken, events.EventIn)
	if err != nil {
